...
WHERE sexo='M';
--Listar nombre y teléfono de los usuarios con teléfono NOKIA, BLACKBERRY o SONY IN sirve para hacer varias condiciones
...
SELECT compañia, count(usuario) AS Miembros FROM tblUsuarios 
GROUP BY compañia;
...
--Listar el login y teléfono de los usuarios con compañia telefónica que no sea TELCEL  sirve para no es igual <>
SELECT usuario,telefono FROM tblUsuarios
...
SELECT usuario,telefono,compañia FROM tblUsuarios
...
--Listar las diferentes marcas de celular en orden alfabético descendentemente DISTINCT--> No repetir String
...
SELECT min(saldo) AS saldoMínimo FROM tblUsuarios
WHERE sexo=lower('H');
